...
export const useCardsStore = defineStore("cards", () => {
  ...
  const addToBasket = (cardId: string) => {
    ...
    return fetch(href, { method: "PUT", credentials: "include", headers: AUTH_DATA_HEADERS }).then(
      (res) => _checkRes<userResp>(res)
    );
  };
  ...
  const delFromBasket = (cardId: string) => {
    ...
    }).then((res) => _checkRes<userResp>(res));
  };
  ...
  const fetchClearBasket = (): userResp => {
    if (!userStore.isLoggedIn) {
      ...
      userStore.updateBasket([]).then((user) => {
        userStore.user = user;
        basketArray.value = user.basket;
      });
    }
