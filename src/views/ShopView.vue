...
const filterFunctions = {
  isBasket: (value, filter) => (filter ? value.productName.includes("корзин") : false),
  minPrice: (value, minPriceValue) => value.price >= minPriceValue,
  maxPrice: (value, maxPriceValue) => value.price <= maxPriceValue,
  isBag: (value, filter) => (filter ? value.productName.includes("сум") : false),
  isTray: (value, filter) => (filter ? value.productName.includes("поднос") : false),
...
const filterCards = () => {
  ...
  const filteredArray = cardsStore.cardsArray.filter((item) => {
    ...
    filterkeys.forEach((key) => {
      if (!filterFunctions[key](item, filters[key])) pass[key] = false;
    });

    if (!filters.isBag && !filters.isBasket && !filters.isTray && !filters.isSmall)
      return pass.minPrice && pass.maxPrice;
    return (
      (pass.isBasket || pass.isTray || pass.isBag || pass.isSmall) && pass.minPrice && pass.maxPrice
    );
  });
